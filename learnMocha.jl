...
net = Net("var-train", backend, [data_layer, common_layers..., loss_layer])

exp_dir = "snapshots"

params = SolverParameters(max_iter=1000, regu_coef=0.0005,
...
# save snapshots every 500 iterations
add_coffee_break(solver, Snapshot(exp_dir), every_n_iter=5000)

# show performance on test data every 1000 iterations
data_layer_test = HDF5DataLayer(name="test-data", source="data/test.txt", batch_size=100)
acc_layer = AccuracyLayer(name="test-accuracy", bottoms=[:ip2, :label])
test_net = Net("var-test", backend, [data_layer_test, common_layers..., acc_layer])
